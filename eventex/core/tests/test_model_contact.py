...
class ContactModelTest(TestCase):
    ...
    def test_email(self):
        """Must create a speaker email contact"""
        contact = Contact.objects.create(speaker = self.speaker, kind = Contact.EMAIL, value = 'henrique@bastos.net')
        self.assertTrue(Contact.objects.exists())

    def test_phone(self):
        """Must create a speaker phone contact"""
        contact = Contact.objects.create(speaker = self.speaker, kind = Contact.PHONE, value = 'henrique@bastos.net')
        self.assertTrue(Contact.objects.exists())
...
class ContactManagerTest(TestCase):
    def setUp(self):
        ...
        s.contact_set.create(kind=Contact.PHONE, value='61-982027270')
